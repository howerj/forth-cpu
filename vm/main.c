...
int main(void)
{
                ...
                for (j = 0; j < LIN_SZ; j++) {
                        if (s[j] == '\n' || s[j] == '\0') {
                                if (j >= (sizeof(mw) * 8)) {
                                        ...
                                        fprintf(stderr,
                                                "Too few characters on line %d for ram_init.\n",
                                                i);
                                        goto FAIL;
                                }
                        }
                        if (s[j] != '0' && s[j] != '1') {
                                fprintf(stderr,
                                        "On line %d, character number %d, an invalid character (%c) was detected.\n",
                                        i, j, s[j]);
                                goto FAIL;
                        }
        ...
        if ((ram_final = fopen("ram.log", "w")) == NULL) {
                fprintf(stderr,
                        "Could not open output file for final RAM contents.\n");
                goto FAIL;
        }
